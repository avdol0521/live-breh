...
## nmap results:
- `nmap 192.168.48.134 -sV -p-`:
```sh fold title:"generic scan result"
Starting Nmap 7.95 ( https://nmap.org ) at 2025-05-12 11:30 EDT
...
MAC Address: 00:0C:29:95:12:DE (VMware)
Service Info: Hosts:  symfonos.localdomain, SYMFONOS; OS: Linux; CPE: cpe:/o:linux:linux_kernel

Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .
Nmap done: 1 IP address (1 host up) scanned in 13.96 seconds
```
- `nmap -Pn -p- -A -T4 --script vuln 192.168.48.134 -oN Ascan.txt`:
```sh title:"Ascan.txt" fold
# Nmap 7.95 scan initiated Mon May 12 11:31:20 2025 as: /usr/lib/nmap/nmap -Pn -p- -A -T4 --script vuln -oN Ascan.txt 192.168.48.134
Nmap scan report for 192.168.48.134
...
TRACEROUTE
HOP RTT     ADDRESS
1   0.37 ms 192.168.48.134

OS and Service detection performed. Please report any incorrect results at https://nmap.org/submit/ .
# Nmap done at Mon May 12 11:32:08 2025 -- 1 IP address (1 host up) scanned in 48.86 seconds
```
## smb enum:
- [[smbmap]] output: <br>
![[symfonosSMBmapOutput.png]]
- there was a single file called `attention.txt`:
```txt
...
```
## http enum (kinda):
- root page had this painting and nothing else:<br>
![[image.jpg]]
- the greek pantheon huh
- since Zeus is a possible user. cant hurt to assume the other users are named after greek gods as well. lets generate a list of greek god names as usernames
```
zeus
hera
poseidon
...
cerberus
```
- brute forcing ssh didnt work. lets try smb again 
- oh hey i missed a share called helios. lemme try with the given creds
## smb enum 2:
- got in with these creds:
```
helios:qwerty
```
- had 2 files
...
3. Work on /h3l105
```
- tried to access it. doesnt load properly. some hyperlinks lead to `http://symfonos.local/h3l105/`. gonna add it to `/etc/hosts` and see if it loads properly <br>
![[symfonosWordpressSiteWorking.png]]
- hell yeah lets run wpscan
- ran it. got a username:
```sh
[+] admin
...
```
- lets brute force with [[wpscan]]
- NOTHING SHOWED UP <br>
![[FB_IMG_1725463438126.jpg]]
- welp thats a deadend then. lets see if i missed anything
## [[SMTP]] enum:
- nmap result shows [[SMTP]]. its a mail service if i remember correctly. something called `Postfix`?
